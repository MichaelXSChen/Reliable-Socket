...
server_type=start server_idx=$1 group_size=3 config_path=$PWD/nodes.local.cfg mgid=ff0e::ffff:e101:101 dare_log_file=$PWD/dare.log ./x86_64-softmmu/qemu-system-x86_64 /opt/vms/ubuntu-desktop.img -enable-kvm -m 4096 -smp 4 -netdev tap,id=hn0,script=/etc/qemu-ifup,downscript=/etc/qemu-ifown -device e1000,id=e0,netdev=hn0,mac=52:a4:00:12:78:66 -vnc :5
