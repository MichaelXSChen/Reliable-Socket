...
static int serve_new_conn(int sk)
{
    ...
    while (1) {
        ...
        printf("Sent %d, return value of send func: %d\n", val, ret);
        rval = -1;
        ret = read(sk, &rval, sizeof(rval));
        printf("Recv %d, return value of recv func: %d\n\n\n", rval, ret);
        sleep(2);
        val++;
    }
...
static int main_cl(int argc, char **argv)
{
    ...
    ret = inet_aton("10.22.1.2", &cliaddr.sin_addr);
    if (ret < 0) {
